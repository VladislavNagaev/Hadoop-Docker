...
ENV \
    ...
    CORE_CONF_hadoop_http_staticuser_user=${USER:-root} \
    # Переменные HDFS
    HDFS_CONF_dfs_replication=${HDFS_CONF_dfs_replication:-3} \
    HDFS_CONF_dfs_blocksize=${HDFS_CONF_dfs_blocksize:-134217728} \
    ...
    HDFS_CONF_dfs_path=${HDFS_CONF_dfs_path:-/hadoop/dfs} \
    HDFS_CONF_dfs_namenode_name_path=${HDFS_CONF_dfs_path:-/hadoop/dfs}/namenode \
    HDFS_CONF_dfs_datanode_data_path=${HDFS_CONF_dfs_path:-/hadoop/dfs}/datanode \
    HDFS_CONF_dfs_namenode_name_dir=file://${HDFS_CONF_dfs_path:-/hadoop/dfs}/namenode \
    HDFS_CONF_dfs_datanode_data_dir=file://${HDFS_CONF_dfs_path:-/hadoop/dfs}/datanode \
...
RUN \
    ...
    chown -R ${USER}:${GID} ${HDFS_CONF_dfs_path} && \
    chmod -R a+rwx ${HDFS_CONF_dfs_path}
